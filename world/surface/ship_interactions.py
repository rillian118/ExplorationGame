...
Commands using this module:
    surface ships
    surface board <ship name|overlay id>
    surface takeoff <ship name|overlay id>

Boarding now targets a canonical ship Airlock room. It no longer falls back to
moving the caller into the ship object itself.
"""
...
def takeoff_landed_ship(caller, query: str) -> str:
    ...
    try:
        from world.space.shipstate import read_ship_location, write_ship_location
...
def board_landed_ship(caller, query: str) -> str:
    ...
    try:
        ...
        set_current_ship_for_caller(caller, ship)
    except Exception as err:
        return f"Boarding target setup failed: {err}"
